...
//			String key = CryptLib.SHA256("my secret key", 32); //32 bytes = 256 bit
            String key = CryptLib.SHA256(cryptKey, 32);
            String iv = CryptLib.generateRandomIV(16); //16 bytes = 128 bit

            output = _crypt.encrypt(input, key, iv); //encrypt
            output = output.substring(0, 3) + iv + output.substring(3);
            return output;
...
//			System.out.println("decrypted text=" + output);
    ...
    public static String decrypt(String cryptKey, String input) {
        ...
        try {
            ...
            String output = "";
            String iv = input.substring(3, 19);
            input = input.substring(0, 3) + input.substring(19);
            String key = CryptLib.SHA256(cryptKey, 32); //32 bytes = 256 bit

//            String iv = CryptLib.generateRandomIV(16); //16 bytes = 128 bit

//            output = _crypt.encrypt(input, Constants.KEY, iv); //encrypt
//			System.out.println("encrypted text=" + output);
//
            output = _crypt.decrypt(input, key, iv); //decrypt
//
//			System.out.println("decrypted text=" + output);
            return output;
...
//            LogUtils.e("encrypted text=", "" + output);

            output = _crypt.decrypt(output, key, iv); //decrypt
//            LogUtils.e("dcrypt text=", "" + output);
//            System.out.println("decrypted text=" + output);
